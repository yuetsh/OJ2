...
export const aiAnalysis = pgTable(
  ...
  (table) => [
    index("ai_analysis_user_id_3aa23011").using(
      "btree",
...
export const announcement = pgTable(
  ...
  (table) => [
    index("announcement_created_by_id_359ccf50").using(
      "btree",
    ...
    index("announcement_list_idx").using(
      "btree",
      table.visible.asc().nullsLast(),
      table.top.desc().nullsFirst(),
      table.createTime.desc().nullsFirst(),
    ),
    foreignKey({
...
export const contest = pgTable(
  ...
  (table) => [
    index("contest_created_by_id_a763ca7e").using(
      "btree",
      table.createdById.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const flowchartSubmission = pgTable(
  "flowchart_submission",
  {
    ...
    evaluationTime: timestamp("evaluation_time", {
      withTimezone: true,
      mode: "string",
    }),
  ...
  (table) => [
    index("flowchart_problem_time_idx").using(
      "btree",
...
export const message = pgTable(
  ...
  (table) => [
    index("message_recipient_time_idx").using(
      "btree",
      table.recipientId.asc().nullsLast().op("timestamptz_ops"),
      table.createTime.asc().nullsLast().op("int4_ops"),
    ),
    index("message_sender_id_a2a2e825").using(
      "btree",
      table.senderId.asc().nullsLast().op("int4_ops"),
    ),
    index("message_submission_id_2fdf8a47").using(
      "btree",
...
export const exercise = pgTable(
  ...
  (table) => [
    index("exercise_tutorial_id_6fd04055").using(
      "btree",
      table.tutorialId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const optionsSysoptions = pgTable(
  "options_sysoptions",
  {
    id: serial().primaryKey().notNull(),
    key: text().notNull(),
  ...
  (table) => [unique("options_sysoptions_key_key").on(table.key)],
)
...
export const problemsetProblem = pgTable(
  "problemset_problem",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({
      name: "problemset_problem_id_seq",
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: "9223372036854775807",
  ...
  (table) => [
    index("problemset_problem_problem_id_fff2d686").using(
      "btree",
      table.problemId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const problemsetProgress = pgTable(
  "problemset_progress",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({
      name: "problemset_progress_id_seq",
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: "9223372036854775807",
  ...
  (table) => [
    index("problemset_progress_user_id_c8041a80").using(
      "btree",
      table.userId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const reaction = pgTable(
  ...
  (table) => [
    index("reaction_problem_type_idx").using(
      "btree",
      table.problemId.asc().nullsLast().op("int4_ops"),
      table.type.asc().nullsLast().op("int4_ops"),
    ),
    index("reaction_user_id_cfa7f469").using(
      "btree",
      table.userId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const problem = pgTable(
  ...
  (table) => [
    index("problem_contest_visible_idx").using(
      "btree",
      table.contestId.asc().nullsLast().op("bool_ops"),
      table.visible.asc().nullsLast().op("int4_ops"),
    ),
    index("problem_created_by_id_cb362143").using(
      "btree",
      table.createdById.asc().nullsLast().op("int4_ops"),
    ),
...
export const problemTags = pgTable(
  ...
  (table) => [
    index("problem_tags_problemtag_id_72d20571").using(
      "btree",
      table.problemtagId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
      columns: [table.problemId],
      foreignColumns: [problem.id],
    ...
    unique("problem_tags_problem_id_problemtag_id_318459d1_uniq").on(
      table.problemId,
      table.problemtagId,
    ),
  ],
...
export const submission = pgTable(
  ...
  (table) => [
    ...
    index("contest_create_time_idx").using(
      "btree",
     ...
     */
    index("submission_result_37e2f67a").using(
      "btree",
      table.result.asc().nullsLast().op("int4_ops"),
    ),
    index("user_create_time_idx").using(
      "btree",
      table.userId.asc().nullsLast().op("int4_ops"),
      table.createTime.asc().nullsLast().op("timestamptz_ops"),
    ),
    /**
     ...
     */
    index("submission_public_problem_time_idx")
      .using(
        "btree",
     ...
     * 扫表本来就是对的计划。
     */
    index("submission_public_username_trgm_idx")
      .using("gin", table.username.op("gin_trgm_ops"))
      .where(sql`${table.contestId} is null`),
    /**
     ...
     */
    index("submission_public_metrics_idx")
      .using(
        "btree",
    ...
    // 也能用上它——不然那条检查要顺序扫全表。
    index("submission_problemset_id_idx")
      .using("btree", table.problemsetId.asc().nullsLast())
      .where(sql`${table.problemsetId} is not null`),
...
export const tutorial = pgTable(
  ...
  (table) => [
    index("tutorial_created_by_id_07973cab").using(
      "btree",
      table.createdById.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
...
export const userAchievement = pgTable(
  ...
  (table) => [
    index("user_achievement_achievement_id_29db600d").using(
      "btree",
      table.achievementId.asc().nullsLast().op("int8_ops"),
    ),
    index("user_achv_notified_idx").using(
      "btree",
      table.userId.asc().nullsLast().op("int4_ops"),
      table.notified.asc().nullsLast().op("bool_ops"),
    ),
    index("user_achv_time_idx").using(
      "btree",
      table.userId.asc().nullsLast().op("int4_ops"),
      table.unlockTime.desc().nullsFirst().op("timestamptz_ops"),
    ),
    foreignKey({
...
export const userBadge = pgTable(
  "user_badge",
  {
    ...
    earnedTime: timestamp("earned_time", {
      withTimezone: true,
      mode: "string",
    }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  ...
  (table) => [
    index("user_badge_badge_id_92a983e9").using(
      "btree",
      table.badgeId.asc().nullsLast().op("int8_ops"),
    ),
    foreignKey({
...
export const userProfile = pgTable(
  ...
  {
    id: serial().primaryKey().notNull(),
    acmProblemsStatus: jsonb("acm_problems_status")
      .default({})
      .notNull()
...
export const acmContestRank = pgTable(
  "acm_contest_rank",
  {
    id: serial().primaryKey().notNull(),
    submissionNumber: integer("submission_number").default(0).notNull(),
    ...
    submissionInfo: jsonb("submission_info")
      .default({})
      .notNull()
      .$type<Record<string, ContestSubmissionInfo>>(),
    contestId: integer("contest_id").notNull(),
    userId: integer("user_id").notNull(),
  },
  (table) => [
    index("acm_rank_contest_user_idx").using(
      "btree",
      table.contestId.asc().nullsLast().op("int4_ops"),
      table.userId.asc().nullsLast().op("int4_ops"),
    ),
    index("acm_rank_order_idx").using(
      "btree",
      table.contestId.asc().nullsLast().op("int4_ops"),
      table.acceptedNumber.asc().nullsLast().op("int4_ops"),
      table.totalTime.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
      columns: [table.contestId],
      foreignColumns: [contest.id],
...
export const user = pgTable(
  ...
  (table) => [
    ...
    index("user_active_idx").using(
      "btree",
      table.isDisabled.asc().nullsLast(),
      table.lastLogin.desc().nullsFirst(),
    ),
...
export const problemsetBadge = pgTable(
  "problemset_badge",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({
      name: "problemset_badge_id_seq",
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: "9223372036854775807",
  ...
  (table) => [
    index("problemset_badge_problemset_id_6cb6c74f").using(
      "btree",
      table.problemsetId.asc().nullsLast().op("int8_ops"),
    ),
    foreignKey({
...
export const tutorialProgress = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.userId, table.tutorialId],
